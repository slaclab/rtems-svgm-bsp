 ...
 */
MEM_MAP seg_fix[] = {
    { FirmwareCode,	0x00000, 0x00002 },
    { BootImage,	0x00002, 0x00afe },
    { FirmwareCode,	0x00b00, 0x00500 },
    { Free,		0x01000, 0x03000 },
    { Unpopulated,	0x04000, 0x00500 },
    { PCIAddr,		0x80000, 0x7ec00 },
    { PCIConfig,	0xfec00, 0x00300 },
    { SystemIO,		0xffeff, 0x00001 },	/* VGM onboard regs */
};
...
#define tsilli() do {\
	((*((int*)0x200000))++);\
	__asm__ __volatile__ ("dcbst 0, %0; icbi 0, %0; sync"::"r"(0x200000));\
	} while(0)
...
void mm_init(u_long image_size) 
{
	...
	*(unsigned long *)0x200004 = (unsigned long)mm->sdr1;
	{ unsigned long ttmp;
	__asm__ __volatile__("bl 1f; 1: mflr %0":"=r"(ttmp):"0"(ttmp):"lr");
	*(unsigned long *)0x200008 = ttmp;
	__asm__ __volatile__("mfmsr %0":"=r"(ttmp));
	*(unsigned long *)0x20000c = ttmp;
	}
	__asm__ __volatile__("sync");
	if (0) { unsigned long tsill;
	  unsigned char *tsptr=(unsigned char*)mm->sdr1;
#if 1
	  for (tsill=0,tsptr=(unsigned char*)mm->sdr1;
			 tsill<0xa0000;
			 tsill++) {
		  *(unsigned long*)0x200000=tsptr;
		  __asm__ __volatile__ ("dcbf 0, %0; sync"::"r"(0x200000));
		  *tsptr=(unsigned char)0;
		  if ((long)tsptr%32==0) __asm__ __volatile__("dcbst 0,%0; sync"::"r"(tsptr-1));
		  tsptr++;
	  }
#endif
