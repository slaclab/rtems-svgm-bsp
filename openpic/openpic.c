...
static void openpic_safe_writefield(volatile unsigned int *addr, unsigned int mask,
				    unsigned int field)
{
int tsill=1500000,val;
static int ncalls=-1;
if (0xf0010000==(unsigned long)addr && 1==++ncalls) BSP_panic("TSILL call");
    openpic_setfield(addr, OPENPIC_MASK);
    /* wait until it's not in use */
    while (((val=openpic_read(addr)) & OPENPIC_ACTIVITY) && --tsill > 0);
    if (tsill<=0) {
	    char buf[300];
	    extern int sprintf(char *, char *,...);
	    sprintf(buf,
		"openpic_safe_writefield hangs addr %x, mask %x, field %x (read %x)",
		addr, mask, field, val);
	    BSP_panic(buf);
    }
